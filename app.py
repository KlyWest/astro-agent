        ...
        for n_name, n_data in NATAL["planets_natal"].items():
            ...
            for asp_name, asp_deg in ASPECTS.items():
                ...
                if exact <= orb_allowed:
                    # направление движения — упрощённо по положению
...
mode_instruction = ""
    if mode == "intro":
        mode_instruction = "\n\n⚠️ РЕЖИМ: Сейчас составляй ТОЛЬКО приветствие, список транзитов и общую картину дня (пункты 1️⃣2️⃣3️⃣ из структуры). НЕ пиши блоки сфер и Луну без курса — это будет отдельным запросом."
    elif mode == "spheres":
        mode_instruction = "\n\n⚠️ РЕЖИМ: Сейчас составляй ТОЛЬКО блок СФЕР (пункт 4️⃣) и Луну без курса если есть (пункт 5️⃣). НЕ пиши приветствие, список транзитов и общую картину — это уже отправлено отдельно. Начни прямо с 💕 Отношения."
...
2️⃣ СПИСОК ВСЕХ АКТИВНЫХ ТРАНЗИТОВ
...
4️⃣ СФЕРЫ — писать ВСЕ ЧЕТЫРЕ всегда
...
5️⃣ ЛУНА БЕЗ КУРСА (если есть)
🌙 Луна без курса с ЧЧ:ММ до ЧЧ:ММ МСК — лучше не начинать новых дел, не подписывать договоры, не делать важных покупок.
Если нет — блок не пишем.

═══ СТРУКТУРА ПРОГНОЗА НА НЕДЕЛЮ ═══

1️⃣ ПРИВЕТСТВИЕ с датами периода. Смайлики.

2️⃣ КРАТКИЙ ОБЗОР НЕДЕЛИ — 3-5 фраз. Общее настроение, доминирующие тренды.

3️⃣ ПО ДНЯМ — для каждого дня с датой:
— Дата и день недели
...
4️⃣ БЛОК СФЕР ЗА НЕДЕЛЮ (в конце, один раз):
...
1️⃣ ПРИВЕТСТВИЕ с названием месяца. Смайлики.

2️⃣ КРАТКИЙ ОБЗОР МЕСЯЦА — 4-6 фраз. Главные тренды, доминирующие транзиты.
...
4️⃣ БЛОК СФЕР ЗА МЕСЯЦ (в конце, один раз):
Для каждой сферы — тренды и ключевые даты за весь месяц.
💕 Отношения | 💼 Работа | 💚 Здоровье | 💰 Финансы

═══ СТРУКТУРА ПРОГНОЗА НА ГОД ═══

1️⃣ ПРИВЕТСТВИЕ с периодом (12 месяцев от даты запроса). Смайлики.

2️⃣ ОБЩИЕ ТЕНДЕНЦИИ ГОДА — 5-7 фраз. Главные медленные транзиты, глобальные темы.

3️⃣ ПО МЕСЯЦАМ — для каждого месяца:
— Название месяца
— 3-5 фраз: общее настроение, значимые события с датами

4️⃣ БЛОК СФЕР ЗА ГОД (в конце, один раз):
...
1️⃣ Приветствие с датами периода.
2️⃣ Общий обзор периода 3-5 фраз.
3️⃣ Значимые даты и события внутри периода.
4️⃣ Блок сфер за период (в конце, один раз).
