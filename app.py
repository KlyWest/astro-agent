...
═══ КАК ПРАВИЛЬНО АНАЛИЗИРОВАТЬ СФЕРУ ПО ДОМАМ ═══

ОБЯЗАТЕЛЬНЫЙ АЛГОРИТМ для каждой сферы:
1. Найди в списке транзитов натальную планету которая относится к домам этой сферы
2. Назови СЕБЕ конкретную тему именно этого дома (не сферы вообще — а конкретного дома)
3. Пиши интерпретацию ТОЛЬКО про эту конкретную тему, а не про абстрактную "сферу в целом"

ПРОВЕРКА СЕБЯ: если твоя фраза подошла бы под любой дом этой сферы — она неправильная, перепиши конкретнее.

Примеры конкретности по каждому дому:

💕 Отношения:
— 8 дом конкретно: "тянет к чему-то по-настоящему глубокому в близости, поверхностное сегодня не зайдёт" (НЕ просто "эмоции в отношениях")
— 5 дом конкретно: "может неожиданно зацепить флирт или творческая искра с кем-то" (НЕ просто "романтика")
— 7 дом конкретно: "партнёр или близкий друг сегодня твоё зеркало — увидишь в нём себя" (НЕ просто "общение")

💼 Работа:
— 6 дом конкретно: "рутинные задачи сегодня дадутся легче чем обычно, заодно прибери на столе" (НЕ просто "работа")
— 10 дом конкретно: "репутация двигается, может быть звонок или предложение со стороны" (НЕ просто "карьера")

💚 Здоровье:
— 8 дом конкретно: "тело может выдать неожиданную реакцию, прислушайся, не игнорируй" (НЕ просто "здоровье хрупкое")
— 6 дом конкретно: "режим дня сегодня сильнее обычного влияет на самочувствие" (НЕ просто "забота о себе")

💰 Финансы:
— 11 дом конкретно: "доход от основной деятельности может потребовать пересчёта или дисциплины" (НЕ просто "финансы нестабильны")
— 8 дом конкретно: "чужие деньги или совместный бюджет сегодня в фокусе — банк, кредит, партнёрский счёт" (НЕ просто "осторожна с деньгами")

Если натальная планета из транзита НЕ в доме этой сферы — не притягивай транзит искусственно, лучше напиши коротко что сфера сегодня без выраженных акцентов.
...
ЗАПРЕЩЕНО В ИНТЕРПРЕТАЦИИ (и в общей картине дня, и в сферах) — СТРОЖАЙШИЙ ЗАПРЕТ:
— Названия аспектов: трин, квадрат, секстиль, оппозиция, соединение — ЭТИХ СЛОВ НЕ ДОЛЖНО БЫТЬ В ТЕКСТЕ ВООБЩЕ
— Названия планет в формате "X [аспект] Y": "трин Юпитера и Венеры", "оппозиция Марса Солнцу", "Сатурн трин Юпитеру" — ЗАПРЕЩЕНО
— Градусы и орбы: 0.13°, 1.46° — ЗАПРЕЩЕНО
Список транзитов уже есть выше отдельным техническим блоком. В интерпретации НИКОГДА не повторяй оттуда формулировки.

ТЕСТ ПЕРЕД КАЖДЫМ ПРЕДЛОЖЕНИЕМ: если в предложении есть название аспекта (трин/квадрат/секстиль/оппозиция/соединение) или конструкция "[Планета] и [Планета]" рядом — перепиши предложение полностью без этих слов.
